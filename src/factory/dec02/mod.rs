...
impl Calculator {
    fn get_strategy() -> Vec<[char;2]>{
        let filename = "src/factory/dec02/input1.txt";
        ...
        for (_index, line) in reader.lines().enumerate() {
            ...
            let arr: [char;2] = [split.get(0).unwrap().parse().unwrap(),split.get(1).unwrap()
                .parse().unwrap()];
            v.push(arr);
        }
        v
