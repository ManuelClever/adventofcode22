...
impl Solve for Calculator {
    fn solve_1(&self) -> i128 {
        ...
        for rucksack in v {
            items.push(Calculator::find_duplicate(Calculator::get_compartments(rucksack.clone())));
        }
    ...
    fn solve_2(&self) -> i128 {
        ...
        let mut i: usize = 0;
        while i < v.len() {
            let triplet: [&Vec<char>;3] = [v.get(i).unwrap(), v.get(i+1).unwrap(), v.get(i+2)
                .unwrap()];
...
impl Calculator {
    fn get_rucksack_content() -> Vec<Vec<char>>{
        let filename = "src/factory/dec03/input1.txt";
    ...
    fn get_compartments(v: Vec<char>) -> [Vec<char>;2] {
        [v[0 .. (v.len() / 2)].to_vec(), v[(v.len() / 2) .. (v.len())].to_vec()]
    }
    ...
    fn sum_priorities(v: Vec<char>) -> i128 {
        ...
        for c in v {
            ...
            if dec >= 65 && dec <= 90 { //uppercase
                sum += dec - 38;
            } else if dec >= 91 && dec <= 122 { //lowercase
                sum += dec - 96;
            }
        }
        sum
